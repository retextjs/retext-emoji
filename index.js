...
function encode(node, value) {
    ...
    if (encodedEmoji = decodedEmoticons[value]) {
        node.fromString(encodedEmoji);
    }
}
...
function decode(node, value) {
    ...
    if (decodedEmoji = encodedEmoticons[value]) {
        node.fromString(decodedEmoji);
    }
}
...
function onchangeFactory(method) {
    return function () {
        var self = this,
            value = self.toString(),
            startNode, nodes, iterator, node, emoticon;
